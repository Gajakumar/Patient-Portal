...
public class NavigateStory {
	...
	def SelectEncounterElementFromLeftNavOnEncounter(Map props) {
		...
		retryAction({
			...
			TestObject tabExpandedTO = makeTO(
					"//a[@title='${pageTitle}']/following-sibling::ul"
					)

			// If tab already expanded
			if (WebUI.verifyElementVisible(tabExpandedTO, FailureHandling.OPTIONAL)) {
				...
				// Expand tab
				WebUI.waitForElementVisible(
						makeTO("//a[@title='${pageTitle}']/preceding-sibling::span"),
						20
						)
				...
				WebUI.waitForElementVisible(
						makeTO("//a[@title='${pageTitle}']/following-sibling::ul/li/a[starts-with(@title,'${element}')]"),
						20
						)
			}

			KeywordUtil.logInfo("Clicked on Element Page ${pageTitle}")
			...
			WebUI.waitForElementVisible(
					makeTO("//div/span[contains(text(), '${element}')]"),
					20
					)
